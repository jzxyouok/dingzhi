...
--
-- Host: 127.0.0.1
-- Generation Time: 2015-09-29 16:57:08
-- 服务器版本： 5.6.20
-- PHP Version: 5.5.20
...
CREATE TABLE IF NOT EXISTS `ecs_ad` (
  ...
  `img_thumb` varchar(60) NOT NULL COMMENT '图片广告缩略图',
  PRIMARY KEY (`ad_id`),
...
DROP TABLE IF EXISTS `ecs_address`;
CREATE TABLE IF NOT EXISTS `ecs_address` (
...
(1, 'admin', 'info@muhuatech.com', '08ce8efe831b855452f108e3cf38ba19', '1605', 1429818523, 1442810104, '210.21.68.55', 'all', '商品列表|goods.php?act=list,订单列表|order.php?act=list,用户评论|comment_manage.php?act=list,会员列表|users.php?act=list,商店设置|shop_config.php?act=list_edit', '', 0, 0, '\r\n你好\r\n', NULL);
...
CREATE TABLE IF NOT EXISTS `ecs_ad_position` (
  ...
  `is_index` int(1) NOT NULL COMMENT '是否属于商城广告位',
...
(4, '资讯速递', 1, '资讯速递', '资讯速递', 50, 0, 0),
(5, '品牌故事', 1, '品牌故事', 'COZY STEPS(英式名称cozie steps) 是HENAN PROSPER USA.INC;注册拥有的美国品牌。2010年公司被美国隆丰公司授权在由河南隆丰皮草企业有限公司在中国大陆生产与销售。COZY STEPS澳大利亚羊皮毛革两用靴（鞋）系列产品自2005年在美国、澳大利亚上市后，以其奢华、经典、柔软、舒适特点，备受好莱坞女明星和时尚女性的青 睐，迅速风靡北美洲及澳大利亚市场。', 50, 0, 0),
(6, '店铺信息', 1, '店铺信息', '北京大兴王府井	\r\n北京市大兴黄村火神庙商业中心F座商业楼1FCOZY专柜\r\n北京长安王府井	\r\n北京市西城区复兴门外大街15号王府井长安商场一层COZY女鞋专柜\r\n北京崇文门-新世界一期店  \r\n北京市东城区崇文门外大街3号崇文门新世界一期1楼cozy专柜\r\n北京崇文门-新世界二期店	  \r\n北京市东城区崇文门外大街5号崇文门新世界二期B1楼cozy专柜\r\n北京当代商城  \r\n北京市海淀区中关村大街40号3Fcozy专柜\r\n', 50, 0, 0);
...
CREATE TABLE IF NOT EXISTS `ecs_feedback` (
  ...
  `is_niming` tinyint(1) NOT NULL COMMENT '是否匿名',
  PRIMARY KEY (`msg_id`),
  KEY `user_id` (`user_id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 ;
...
--
-- 表的结构 `ecs_feedback_status`
--

DROP TABLE IF EXISTS `ecs_feedback_status`;
CREATE TABLE IF NOT EXISTS `ecs_feedback_status` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `userid` int(11) NOT NULL,
  `goods_id` int(11) NOT NULL,
  `status` tinyint(1) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='商品评价关系表' AUTO_INCREMENT=1 ;
...
--
-- 表的结构 `ecs_search_list`
--

DROP TABLE IF EXISTS `ecs_search_list`;
CREATE TABLE IF NOT EXISTS `ecs_search_list` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
(1, 'post_mail', '邮局平邮', '邮局平邮的描述内容。', '0', 0, 1, '', '', '', 2, 0),
...
(8, 0, 'sms', 'group', '', '', '', 1),
(9, 0, 'wap', 'group', '', '', '', 1),
(101, 1, 'shop_name', 'text', '', '', 'Cozyshop', 1),
(102, 1, 'shop_title', 'text', '', '', 'Cozyshop', 1),
(103, 1, 'shop_desc', 'text', '', '', 'Cozyshop', 1),
(104, 1, 'shop_keywords', 'text', '', '', 'Cozyshop', 1),
...
(115, 1, 'service_phone', 'text', '', '', '10086', 1),
...
(603, 6, 'hash_code', 'hidden', '', '', 'ed39652d0fb0a14d46bd7987fc339144', 1),
(604, 6, 'template', 'hidden', '', '', 'default', 1),
(605, 6, 'install_date', 'hidden', '', '', '1429847325', 1),
...
DROP TABLE IF EXISTS `ecs_url`;
CREATE TABLE IF NOT EXISTS `ecs_url` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
INSERT INTO `ecs_url` (`id`, `name`, `url`) VALUES
(1, 'moble.images', 'http://cozyshop.wxpop.cn/upload/images/'),
(2, 'moble.img', 'http://cozyshop.wxpop.cn/upload/images/');
...
CREATE TABLE IF NOT EXISTS `ecs_users` (
  ...
  `openid` varchar(50) NOT NULL DEFAULT '' COMMENT '微信登录获取的openid',
  PRIMARY KEY (`user_id`),
