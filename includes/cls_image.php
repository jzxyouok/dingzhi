...
class cls_image
{
     ...
     */
    function getThumb($imgsrc,$imgdst,$imgwidth,$imgheight)
    {
        //取得图片的宽度,高度值
        ...
        $imgsrc = $this->img_resource($imgsrc, $org_info[2]);

        $image = imagecreatetruecolor($imgWidth, $imgHeight); //创建一个彩色的底图
        imagecopyresampled($image, $imgsrc, 0, 0, 0, 0,$imgWidth,$imgHeight,$arr[0], $arr[1]);
        $filename .= '.jpg';
        imagejpeg($image, $dir . $filename);
        imagedestroy($image);//释放处理库内存
        $filename='images/'.date('Ym').'/'.$filename;
        return $filename;
    }
}
