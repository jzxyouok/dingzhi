...
if ($_REQUEST['act'] == 'insert')
{
    ...
    if(!empty($_FILES)){
        $img   = $image->upload_image($_FILES['article_thumb']); // 原始图片
        $thumb=  $image->make_thumb('../'. $img , 400,  400);
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, cat_id, article_type, is_open, author, ".
                "author_email, keywords, content, add_time, file_url, open_type, link, description,img,thumb) ".
            "VALUES ('$_POST[title]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', ".
                ...
                "'$add_time', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]','$img','$thumb')";
    $db->query($sql);
