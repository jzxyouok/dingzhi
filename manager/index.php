...
//    $today_start = mktime(0,0,0,date('m'),date('d'),date('Y'));
    ...
    $order_stats = array();
    if (date('Ym', $month_start) <= date('Ym')) {
        //日子大过今天的都不显示
        $order_list = $db->getAll('SELECT * FROM ' . $ecs->table('order_stats') . ' WHERE (date >= '.$month_start.' AND date <= '.$month_end.') ORDER BY rec_id ASC');
        $today_day = date('Ymd');
        $today_month_start_day = date('Ymd', $month_start);
        $month_total_day = date('d', $month_end);
        for($i = 0;$i < $month_total_day && $today_month_start_day <= $today_day;$i++) {
            $order_stats[] = array('text'=>'0单');
            $today_month_start_day++;
        }
        if ($order_list) {
            foreach ($order_list as $temp) {
                $order_stats[date('d',$temp['date'])-1] = array('text'=>$temp['order_count'].'单');
            }
        }
    }
    $smarty->assign('month', $month);
    $smarty->assign('prev_month', date('Y-m', strtotime('-1 month', $month_start)));
    $smarty->assign('next_month', date('Y-m', strtotime('+1 month', $month_start)));
    $smarty->assign('max_order_count', $db->getOne('SELECT value FROM ' . $ecs->table('shop_config') . ' WHERE code = \'max_order_count\''));
    $smarty->assign('order_stats', json_encode($order_stats));
...
elseif ($_REQUEST['act'] == 'set_max_order') {
    $max_order = intval($_POST["max_order"]);
    $sql = "UPDATE" .$GLOBALS['ecs']->table('shop_config'). " SET value='" . $max_order . "' WHERE code = 'max_order_count'";
...
elseif ($_REQUEST['act'] == 'main_api')
{
    ...
    ' WHERE is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
        ...
        $apiget = "ver= $ecs_version &lang= $ecs_lang &release= $ecs_release &php_ver= $php_ver &mysql_ver= $mysql_ver &charset= $ecs_charset &patch= $patch_file &style= $ecs_style ";//david
