...
if ($_REQUEST['act'] == 'enroll'){
    admin_priv('topic_manage');
    $smarty->assign('ur_here',    '专题活动报名管理');
    $smarty->assign('full_page',   1);
    $topic_id  = empty($_REQUEST['topic_id']) ? 0 : intval($_REQUEST['topic_id']);
    $list = get_enroll_list($topic_id);

    $smarty->assign('enroll_list',   $list['item']);
    ...
    assign_query_info();
    $smarty->assign('action_link', array('text' => $_LANG['topic_list'], 'href' => 'topic.php?act=list'));
    $smarty->display('enroll_list.htm');
...
elseif ($_REQUEST["act"] == "delete_enroll")
{
    admin_priv('topic_manage');
    $sql = "DELETE FROM " . $ecs->table('topic_enroll') . " WHERE ";

    if (!empty($_POST['checkboxes']))
    {
        $sql .= db_create_in($_POST['checkboxes'], 'id');
    }
    elseif (!empty($_GET['id']))
    {
        $_GET['id'] = intval($_GET['id']);
    ...
    if (!empty($_REQUEST['is_ajax']))
    {
        $url = 'topic.php?act=enroll_query&' . str_replace('act=delete_enroll', '', $_SERVER['QUERY_STRING']);
        ecs_header("Location: $url\n");
        exit;
    }
...
elseif ($_REQUEST["act"] == "enroll_query")
{
    $enroll_list = get_enroll_list(intval($_REQUEST['topic_id']));
    $smarty->assign('enroll_list',   $enroll_list['item']);
    $smarty->assign('filter',       $enroll_list['filter']);
    $smarty->assign('record_count', $enroll_list['record_count']);
    $smarty->assign('page_count',   $enroll_list['page_count']);
    $smarty->assign('use_storage',  empty($_CFG['use_storage']) ? 0 : 1);

    /* 排序标记 */
    $sort_flag  = sort_flag($enroll_list['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);

    $tpl = 'enroll_list.htm';
    make_json_result($smarty->fetch($tpl), '',array('filter' => $enroll_list['filter'], 'page_count' => $enroll_list['page_count']));
}
/**
 * 获取专题列表
 * @access  public
...
function get_enroll_list($topic_id){
    ...
    if ($result === false)
    {
        ...
        $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'topic_id' : trim($_REQUEST['sort_by']);
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
		$filter['topic_id'] = empty($_REQUEST['topic_id']) ? 0 : intval($_REQUEST['topic_id']);
		 $where = empty($filter['topic_id']) ? '' : " WHERE topic_id='$filter[topic_id]'";
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('topic_enroll')." $where ";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT t.*,u.* FROM " .$GLOBALS['ecs']->table('topic_enroll')." AS t ".
